<?php
    ...
    $bulan = $ex[0];
    $tahun = $ex[1];
    $sql_b = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi`, `u`.`id_user` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user` WHERE MONTH(`b`.`tanggal`) = $bulan AND YEAR(`b`.`tanggal`) = $tahun ORDER BY `b`.`judul` LIMIT $posisi, $batas";       
...
<div class="col-sm-12">
    <nav aria-label="Page navigation">
        <?php 
            $sql_b = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user` WHERE MONTH(`b`.`tanggal`) = $bulan AND YEAR(`b`.`tanggal`) = $tahun ORDER BY `b`.`judul`";
