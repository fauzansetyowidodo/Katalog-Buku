      ...
      <main role="main" class="container">
        <div class="row">
          <div class="col-md-9 blog-main">
            <?php
              $sql_bl = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi`, `u`.`id_user` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user`";
        ...
        //hitung jumlah semua data
           $sql_jum = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi`, `u`.`id_user` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user` ";
