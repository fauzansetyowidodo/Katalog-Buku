<?php
    ...
    $sql_b = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user` WHERE `b`.`id_kategori_blog` = '$data' ORDER BY `b`.`judul` LIMIT $posisi, $batas";       
    ...
    if($row == 0 ){
        ...
        ?>
         <div class="blog-post">
              ...
              <p class="blog-post-meta"><?php echo $tanggal_blog; ?> by <a href="#"><?php echo $nama_blog; ?></a></p>
...
<div class="col-sm-12">
    <nav aria-label="Page navigation">
        <?php 
            $sql_b = "SELECT `b`.`id_blog`,`b`.`judul`, DATE_FORMAT(`b`.`tanggal`, '%M %d, %Y'),`u`.`nama`, `b`.`isi` FROM `blog` `b` INNER JOIN `user` `u` ON `b`.`id_user`=`u`.`id_user` WHERE `id_kategori_blog` = '$data' ORDER BY `b`.`judul`";
