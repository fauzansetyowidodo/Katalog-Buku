...
if(isset($_GET['id'])){
	...
	//get data blog
	$sql = "SELECT `kb`.`kategori_blog`,`u`.`nama`,`b`.`judul`,`b`.`isi`,`b`.`tanggal` FROM `blog` `b` INNER JOIN `kategori_blog` `kb` ON `b`.`id_kategori_blog`=`kb`.`id_kategori_blog` INNER JOIN `user` `u` ON `b`.`id_user` WHERE `b`.`id_blog`='$id_blog'";
